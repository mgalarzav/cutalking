...
const FeedbackModal: React.FC<Props> = ({ data, scenarioTitle, onClose }) => {
    ...
    const playAudio = async (text: string, id: string) => {
        ...
        try {
            // Try Groq TTS
            let arrayBuffer = await groqService.generateSpeech(text, settings.avatar);

            // Fallback to Deepgram
            if (!arrayBuffer) {
                console.warn("Feedback: Groq TTS failed, attempting Deepgram fallback...");
                arrayBuffer = await deepgramService.generateSpeech(text, settings.avatar);
            }
            ...
            if (arrayBuffer) {
                ...
                console.error("Feedback: All TTS services failed.");
                setPlayingId(null);
            }
        } catch (e) {
                                ...
                                {data.practicePhrases.map((phrase, idx) => (
                                    <div key={idx} className="bg-white/80 dark:bg-slate-800/80 p-3 rounded-xl flex items-center justify-between shadow-sm">
                                        ...
                                        <button
                                            ...
                                            className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-bold transition-all ${playingId === `prac-${idx}`
